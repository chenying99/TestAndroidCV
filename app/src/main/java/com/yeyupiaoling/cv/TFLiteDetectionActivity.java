...
import com.yeyupiaoling.cv.lite.TFLiteDetection;
...
public class TFLiteDetectionActivity extends AppCompatActivity {
    private static final String TAG = TFLiteDetectionActivity.class.getName();
    private TFLiteDetection tfLiteDetection;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tflite_detection);
        String detection_model_path = getCacheDir().getAbsolutePath() + File.separator + "ssd_mobilenet_v1.tflite";
        Utils.copyFileFromAsset(TFLiteDetectionActivity.this, "ssd_mobilenet_v1.tflite", detection_model_path);
        try {
            tfLiteDetection = new TFLiteDetection(detection_model_path);
        } catch (Exception e) {
            e.printStackTrace();
            Log.d(TAG, "模型加载失败");
            ...
            new Runnable() {
                ...
                public void run() {
                    synchronized (lock) {
                        if (runClassifier) {
                            // 开始预测前要判断相机是否已经准备好
                            if (getApplicationContext() != null && mCameraDevice != null && tfLiteDetection != null) {
                                detection();
                            }
                        }
                    }
    ...
    private void detection() {
        ...
        try {
            ...
            List<float[]> results = tfLiteDetection.predictImage(bitmap);
